#pragma once
#include "illixr/network/network_backend.hpp"
#include "illixr/network/tcpsocket.hpp"
#include "illixr/network/topic_config.hpp"
#include "illixr/plugin.hpp"
#include "illixr/switchboard.hpp"
#include <thread>
#include <atomic>
#include <vector>
#include <string>
#include <unordered_map>
#include <algorithm>

namespace ILLIXR {

class tcp_minimal_backend
    : public plugin
    , public network::network_backend {
public:
    explicit tcp_minimal_backend(const std::string& name_, phonebook* pb_);
    
    // network_backend interface
    void topic_create(std::string topic_name, network::topic_config& config) override;
    bool is_topic_networked(std::string topic_name) override;
    void topic_send(std::string topic_name, std::string&& message) override;
    void stop() override;

private:
    void start_client();
    void start_server();
    void read_loop(network::TCPSocket* socket);
    void read_loop_reverse(); // For relay's reverse path (end server → client)
    void topic_receive(const std::string& topic_name, std::vector<char>& message);
    void send_to_peer(const std::string& topic_name, std::string&& message);

    std::shared_ptr<switchboard> switchboard_;
    
    std::atomic<bool> running_{true};
    std::atomic<bool> ready_{false};
    
    // Sockets
    network::TCPSocket* peer_socket_{nullptr};     // Connection to peer
    network::TCPSocket* forward_socket_{nullptr};  // Relay's connection to end server
    
    // Configuration
    std::string server_ip_;
    int         server_port_{0};
    std::string client_ip_;
    int         client_port_{0};
    int         is_client_{0};
    std::string forward_ip_;
    int         forward_port_{0};
    
    // Topic management
    std::vector<std::string>                               networked_topics_;
    std::unordered_map<std::string, network::topic_config> networked_topics_configs_;
    std::string delimiter_{":"};
};

} // namespace ILLIXR
